...
run : $(APPL_FULL_NAME).elf
	@$(ECHO) "Download & Run $<"
#
# only gnu coverage test is support.
# the coverage data will be output through console and processed
# by gen_gcov_files.py
#
ifeq ($(EN_COVERAGE)$(BOARD)$(VALID_TOOLCHAIN),1nsimgnu)
	$(DBG) $(DBG_HW_FLAGS) $< $(CMD_LINE) >> $(APPL_FULL_NAME).log
	python $(EMBARC_ROOT)/scripts/gen_gcov_files.py -i $(APPL_FULL_NAME).log
else
...
$(APPL_FULL_NAME).elf : $(ALL_GENERATED_DIRS_TMPFILES) $(CORE_ARG_FILES) $(APPL_LINK_FILE) $(APPL_OBJS) $(EXTRA_OBJS) $(EMBARC_LIB) $$(COMMON_COMPILE_PREREQUISITES)
	$(TRACE_LINK)
	$(Q)$(LD) $(LINK_OPT) $(APPL_OBJS) $(LD_START_GROUPLIB) $(EXTRA_OBJS) $(EMBARC_LIB) $(APPL_LIBS) $(LD_SYSTEMLIBS) $(LD_END_GROUPLIB) -o $@

#####RULES FOR ACHIEVING EMBARC WHOLE LIBRARY#####
$(EMBARC_LIB) : $(ALL_GENERATED_DIRS_TMPFILES) $(CORE_ARG_FILES) $(EXTRA_OBJS) $(ALL_LIBS) $(APPL_LINK_FILE) $$(COMMON_COMPILE_PREREQUISITES)
	$(TRACE_ARCHIVE)
	$(Q)-$(IFEXISTFILE) $(subst /,$(PS),$(EMBARC_LIB)) $(ENDIFEXISTFILE) $(RM) $(subst /,$(PS),$(EMBARC_LIB))
	$(Q)$(AR) $(AR_OPT) $@ $(EXTRA_OBJS) $(ALL_LIBS)
